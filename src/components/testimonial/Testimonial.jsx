/* eslint-disable react/no-unescaped-entities */

const Testimonial = () => {
    return (
        <div>
            <section className="text-gray-600 body-font mb-10">
                {/* main  */}
                <div className="container px-5 py-10 mx-auto">
                    {/* Heading  */}
                    <h1 className='text-center text-3xl font-bold text-black'>Customer Testimonials</h1>
                    {/* para  */}
                    <h2 className='text-center text-2xl font-semibold mb-10'>What our <span className='text-gray-900'>diners</span> are saying</h2>

                    <div className="flex flex-wrap -m-4">
                        {/* Testimonial 1 */}
                        <div className="lg:w-1/3 lg:mb-0 mb-6 p-4">
                            <div className="h-full text-center">
                                <img alt="testimonial" className="w-20 h-20 mb-8 object-cover object-center rounded-full inline-block border-2 border-gray-200 bg-gray-100" src="https://cdn-icons-png.flaticon.com/256/9131/9131529.png" />
                                <p className="leading-relaxed">The atmosphere at OurRestaurant is simply amazing. Every time I visit, I'm blown away by the cozy ambiance and the delicious food. It's truly a gem in the heart of the city.</p>
                                <span className="inline-block h-1 w-10 rounded bg-gray-900 mt-6 mb-4" />
                                <h2 className="text-gray-900 font-medium title-font tracking-wider text-sm uppercase">Vikas R</h2>
                                <p className="text-gray-900">Food Enthusiast</p>
                            </div>
                        </div>

                        {/* Testimonial 2 */}
                        <div className="lg:w-1/3 lg:mb-0 mb-6 p-4">
                            <div className="h-full text-center">
                                <img alt="testimonial" className="w-20 h-20 mb-8 object-cover object-center rounded-full inline-block border-2 border-gray-200 bg-gray-100" src="https://www.devknus.com/img/gawri.png" />
                                <p className="leading-relaxed">My recent dining experience at OurRestaurant was nothing short of exceptional. From the warm welcome at the door to the delectable dishes served, everything was top-notch. I can't wait to visit again!</p>
                                <span className="inline-block h-1 w-10 rounded bg-gray-900 mt-6 mb-4" />
                                <h2 className="text-gray-900 font-medium title-font tracking-wider text-sm uppercase">Sanjana</h2>
                                <p className="text-gray-900">Fine Dining Enthusiast</p>
                            </div>
                        </div>

                        {/* Testimonial 3 */}
                        <div className="lg:w-1/3 lg:mb-0 p-4">
                            <div className="h-full text-center">
                                <img alt="testimonial" className="w-20 h-20 mb-8 object-cover object-center rounded-full inline-block border-2 border-gray-200 bg-gray-100" src="data:image/png;base64," />
                                <p className="leading-relaxed">I recently had the pleasure of dining at OurRestaurant, and I must say, it was a fantastic experience from start to finish. From the diverse menu options to the attentive service, everything about my visit exceeded my expectations.al </p>
                                <span className="inline-block h-1 w-10 rounded bg-gray-900 mt-6 mb-4" />
                                <h2 className="text-gray-900 font-medium title-font tracking-wider text-sm uppercase">Monish</h2>
                                <p className="text-gray-900">IT Employee</p>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </div>
    )
}

export default Testimonial